...
**Goal:** Wire the tested voxpen-core business logic and React UI together through Tauri IPC commands, concrete hardware implementations, global hotkey handling, and state event emission — making VoxPen Desktop a working end-to-end voice dictation app.

**Architecture:** The Tauri app crate (`src-tauri/src/`) provides concrete implementations of the hardware traits defined in voxpen-core (AudioRecorder via cpal, ClipboardManager via arboard, KeySimulator via enigo), Tauri IPC commands that the React frontend calls via `invoke()`, and a global hotkey handler that drives the PipelineController state machine. State changes are broadcast to React via Tauri events.
...
**Step 2: Verify voxpen-core tests still pass**
...
use voxpen_core::history::{self, TranscriptionEntry};
use voxpen_core::pipeline::settings::Settings;
...
use voxpen_core::pipeline::controller::PipelineController;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let quit_item =
                MenuItem::with_id(app, "quit", "Quit VoxPen", true, None::<&str>)?;
...
- voxpen-core tests (89 tests) remain the validation mechanism.
